...
`code-oz` is a standalone Bun + TypeScript CLI that boots an adaptive multi-agent software-delivery runtime over a hybrid phase-graph + agentic sub-orchestration spine. Hard SDLC gates between phases (file-based, schema-validated). Cross-family adversarial review. Non-technical-user intent elicitation at the front. Provider surface (v0.20.1): Claude (Claude Code CLI subprocess), Codex (Codex CLI subprocess), xAI (direct HTTPS with `XAI_API_KEY`), Fake (built-in deterministic) — all four live. Gemini is a stub for transparency only (`invoke()` throws `provider_gemini_not_yet_supported`); OpenCode and Roo Code are future adapter candidates, not v0.1 implementations. The canonical provider matrix lives at `docs/contracts/PROVIDERS.md` § "Provider status (v0.1)".

Status: **v0.20.0-alpha.0 shipped (npm + Homebrew + curl); preparing v0.20.1-alpha.0 (first-run polish per `docs/design/V0_20_1_FIRST_RUN_POLISH_DESIGN.md`).** v0.20.1 pulls forward the locked Option D Phase 3 trust/community/proof tasks ahead of M17 AUDIT runtime (v0.21). v0.20.1 scope: README hero rewrite to "CI-style gates for AI coding agents"; provider claims corrected (no Gemini overclaim); `SECURITY.md` / `CONTRIBUTING.md` / `CODE_OF_CONDUCT.md` / `docs/TRUST.md` added; `.github/` issue templates + PR template; failure-mode demo `docs/demo/02-failure-gates/` with five fixtures + characterization tests asserting each production gate API refuses the wrong input (per-fixture `events-sketch.jsonl` files are author-constructed summaries, not real production events); `docs/comparisons/ai-coding-agents.md` (Option D §3.2 verbatim); `docs/benchmarks/agent-gate-bench.md` protocol-only (runner deferred to v0.21); `docs/design/ROADMAP.md` Now/Next/Later top section. No new gate authority introduced (rule 20). Cross-family review at the failure-demo code track + one public-claims bundle + final pre-tag (Codex `gpt-5.5` xhigh). v0.20 distribution surface unchanged from W3a (same four per-arch binaries, same install script, same npm wrapper, same Homebrew tap). Prior milestones (M14 Reviewer panel + M15 debate-policy scheduler + M16 production CLI completion authorities + the 23 non-negotiable rules below) all hold. **3395 offline tests pass** (live xAI integration gated behind `CODE_OZ_LIVE_PROVIDER_TESTS=xai` + `CODE_OZ_LIVE_XAI_MODEL=<grok-variant>`). PE-2 demand-gated; multi-cloud deferred to v0.2.
...
- `docs/comparison/<NN>-<template>/` — per-template head-to-head comparisons against the influence library, each closed with a Codex round (e.g., `09-byterover-cli/SYNTHESIS.md` produced rule 22)
