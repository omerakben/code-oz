...
Status: **v0.12.0-alpha.0 — M12 closed (Company roster: locked six-name `M12_COMPANY_ROLES` constant + `CompanyConfig = { provider?, model? }` rows only with fail-closed rejection of unsupported row keys + `loader_company_role_unknown` config-load check against the locked roster + `applyCompanyOverrides` between bundled-vs-override merge and the resolved-provider checks + post-override debate-family re-check that re-asserts the schema-time `opposingProviders` invariant against the resolved family (Codex Risk #4) + bootstrap-before-loadConfig wiring fix in `src/cli/bootstrap.ts` and `src/commands/run.ts` at both call sites (Codex Risk #2) + `req.model ?? req.agent.model` defaulting in `src/providers/manifest.ts` + `agent_invoked.model` event field in `src/state/schemas.ts` for durable provenance (Codex Risk #3, also closes the latent frontmatter-model-drop bug that pre-dated M12))**. 1917 offline tests pass. PE-1 next: xAI direct HTTP adapter. Read `docs/design/ROADMAP.md` first.
