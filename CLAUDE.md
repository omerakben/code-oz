...
Status: **v0.10.0-alpha.0 — M10 closed (Debate runtime + `requestDebate()` primitive: tool_use.debate schema with cross-family load-time invariant + debate_started/debate_resolved events with forward-compat correlation metadata + ignore-policy module with fail-closed unsupported syntax + BRIEFING/RESPONSE/DECISION parser+serializer+canonicalizer + non-interactive manifest preview audit + opponent+synthesis prompts with universal-rules injection + requestDebate orchestrator with three-call budget accounting + lead persona declarations)**. 1761 offline tests pass. M11 next: Provider capability contract. Read `docs/design/ROADMAP.md` first.
