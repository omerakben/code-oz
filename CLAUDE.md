...
Status: **v0.12.0-alpha.0 — M12 closed.** The `.code-oz/config.yaml` `company:` block routes the six bundled personas (locked `M12_COMPANY_ROLES`) via `{ provider?, model? }` overrides; per-role budgets defer to M13; permissions stay persona-shaped. 1917 offline tests pass; review trail in `docs/research/CODEX_REVIEW_M12.md` (closes risks #2/#3/#4) and the inter-milestone refactor session in `docs/research/REFACTOR_AUDIT_2026-05-01.md` (closes the two M12 deferred risks). PE-1 next: xAI direct HTTP adapter. Read `docs/design/ROADMAP.md` first.
