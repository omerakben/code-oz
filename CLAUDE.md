...
Status: **v0.19.0-alpha.0 shipped; preparing v0.20.0-alpha.0 (W3a distribution sweep).** v0.20 ships the multi-channel distribution surface locked by the W3a synthesis (`docs/design/CODEX_SYNTHESIS_W3A.md`): four per-arch native binaries (darwin-{arm64,x64} + linux-{x64,arm64}) compiled via `bun build --compile` in CI, a fail-closed `scripts/install.sh` (SHA chain `sha256sum → shasum → openssl`, Linux detection, network-mode fetch via `--version <TAG>` against the tagged release URL with checksums.txt verification), the project's first GitHub Actions workflows (`test.yml` matrix + `release.yml` 4-target build + checksums + asset upload), an npm Node-launcher wrapper (`npm-wrapper/index.cjs`; downloads + SHA-verifies + caches at `~/.cache/code-oz/<version>/`), and a Homebrew formula template at `docs/homebrew/code-oz.rb.template` rendered into the `omerakben/homebrew-code-oz` tap at release time. **Scope amendment locked**: Scoop + Windows binary deferred to v0.20.1; Apple Developer signing + GPG-signed checksums deferred to v0.x stable. The single-binary contract holds across all three channels (curl|sh, Homebrew, npm) — same SHA, same artifact, same `checksums.txt` source-of-truth. Prior milestones (M14 Reviewer panel + M15 debate-policy scheduler + M16 production CLI completion authorities + the 23 non-negotiable rules below) all hold. **3353 offline tests pass** (live xAI integration gated behind `CODE_OZ_LIVE_PROVIDER_TESTS=xai` + `CODE_OZ_LIVE_XAI_MODEL=<grok-variant>`). PE-2 demand-gated; multi-cloud deferred to v0.2.
