...
Status: **v0.11.0-alpha.0 — M11 closed (Provider capability contract: strict-minimal `ProviderCapability` shape with `authSource` mechanism enum, `eligiblePhases`, optional advisory `costPerMTok` + `rateLimits` + `src/providers/capabilities.ts` defaults module paralleling `families.ts` + `IAgentProvider.capability` static field on every adapter + `ProviderRegistry.capabilityOf` + `capabilityOverrides` constructor seam with structural-equality cross-check + `enforceProviderPhaseEligibility` load-time check including `tool_use.debate.opposingProviders` walk to close M10 synthetic-opponent bypass)**. 1860 offline tests pass. M12 next: Company roster (shipped roles only). Read `docs/design/ROADMAP.md` first.
