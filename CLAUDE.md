...
Status: **v0.18.0-alpha.0 — template-comparison sweep landed; preparing v0.19.0-alpha.0.** 17 PRs merged on 2026-05-11 closing the full 22-template comparison series + 12 substantive borrows: rule 22 (consumer-first + RED-first TDD), rule 1 intervention-writer authority expansion, rule 9 generalized to any executable runner, rule 16 persona-generation discipline (Mimir C-MIMIR-4) + 21-item universal-rules expansion, named approval presets (auto/paranoid/interactive), REVIEW specialist rubric, lintSpecQuality DEFINE warnings (M-SPEC1), allowlisted env reader (pi-mono B5), cross-family handoff matrix test (pi-mono B4), parentTaskId fan-out cost rollup (byterover B3), actor-attribution discipline on all events (Chorus §3.5), guardrails fail-open posture + CRLF tolerance + validationOutcome round-trip (claude-code), codegraph runner symbol-guard hardening + Windows portability, ADR gate + architecture vocabulary affordances (M18b partial), agent-skills PLAN bug-fix bullet (B6). On 2026-05-12 the locked 3-session plan landed `--effort lite|balanced|max|beast` (B1a, rule 23), the opencode triage merge (MCP_TRUST_BOUNDARY.md + 2 candidate slots), and a runnable end-to-end demo (`scripts/demo/01-todo-cli/run-demo.ts` + 3 effort captures under `docs/demo/01-todo-cli/output/`). M17-M20 candidate slots reserved for gptme-derived borrows; 2 opencode candidate slots reserved (deny-dominant wildcard, M-CANCEL). M14 Reviewer panel + M15 debate-policy scheduler + M16 production CLI completion authorities all hold. **3299 offline tests pass** (live xAI integration gated behind `CODE_OZ_LIVE_PROVIDER_TESTS=xai` + `CODE_OZ_LIVE_XAI_MODEL=<grok-variant>`). PE-2 demand-gated; multi-cloud deferred to v0.2.
