...
The full v0.20.1 first-run polish + public truth sync release adds three small `src/` first-run polish fixes (provider auth-expired classification, intervention-pointer specificity, CLI first-run-fake + resume-paths) plus the README + security + community + failure-demo + benchmark protocol work. See [`v0.20.1-alpha.0`](https://github.com/omerakben/code-oz/releases/tag/v0.20.1-alpha.0) when published. No new gate authority is introduced; provider contract unchanged.
