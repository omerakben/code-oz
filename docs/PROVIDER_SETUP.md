...
`code-oz` has one first-run rule: start cost-free unless you explicitly opt into a live provider. In v0.20.1, no-key CLI runs use `FakeProvider` automatically.
...
| Claude CLI family | Claude CLI login | Live Claude-backed personas use the installed CLI's login/session | No direct `ANTHROPIC_API_KEY` adapter ships in v0.20.1. |
| Codex CLI family | Codex CLI login | Cross-family REVIEW uses the installed CLI's login/session | No direct `OPENAI_API_KEY` adapter ships in v0.20.1. |
