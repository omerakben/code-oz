...
A2. **Delta operations over rewrite-the-context**. ACE's Curator emits a JSON list typed as `{type: "ADD" | "UPDATE" | "MERGE" | "DELETE" | "CREATE_META", section, content}`. The published `playbook_utils.apply_curator_operations` only implements `ADD` today (`playbook_utils.py:96-141`); `UPDATE` / `MERGE` / `DELETE` / `CREATE_META` are TODO branches and the curator prompt at `prompts/curator.py:49-53` exposes only `ADD`. The deltas-over-rewrite *principle* is the borrow target — prevents "context collapse" from iterative rewriting eroding details. code-oz currently has no analog because there is no document to mutate. See SYNTHESIS.md finding 1 for the ADD-only acceptance scope.
