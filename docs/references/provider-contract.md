...
1. **No shell-string execution.** `Bun.spawn` is the only execution primitive code-oz uses, and only with explicit `argv` arrays. There is no `sh -c "<user-string>"` path anywhere in the codebase, and there will not be one. If a future feature needs shell composition, it lands as a typed builder that produces an argv array, never a free-form string.
