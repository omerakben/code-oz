...
`code-oz` ships five gate-class files. The first three are intervention gates inherited from maestro; the last two are `code-oz` extensions for typed phase progression and run trace.

### 1. `state/NEEDS_INTERVENTION.json` (intervention)
...
### 2. `state/PAUSE.json` (intervention)
...
### 3. `state/STOP.json` (intervention)
...
### 4. `state/GATE_<PHASE>_PASSED.json` (success — `code-oz` extension)
...
### 5. `state/events.jsonl` (run trace — `code-oz` extension)
...
```json
{ "type": "run_started",      "ts": "...", "runId": "...", "profile": "greenfield" }
{ "type": "phase_entered",    "ts": "...", "runId": "...", "phase": "define" }
{ "type": "phase_exited",     "ts": "...", "runId": "...", "phase": "define", "outcome": "passed" }
{ "type": "agent_invoked",    "ts": "...", "runId": "...", "phase": "define", "agent": "ba", "provider": "claude" }
{ "type": "agent_completed",  "ts": "...", "runId": "...", "phase": "define", "agent": "ba", "tokensUsed": 1834 }
{ "type": "gate_written",     "ts": "...", "runId": "...", "phase": "define", "file": "state/GATE_DEFINE_PASSED.json" }
{ "type": "gate_required",    "ts": "...", "runId": "...", "phase": "define", "blockedOn": "user signoff" }
{ "type": "intervention",     "ts": "...", "runId": "...", "code": "...", "phase": "..." }
{ "type": "run_ended",        "ts": "...", "runId": "...", "outcome": "shipped" | "stopped" | "paused" }
```

Required on every event: `type`, `ts` (ISO 8601), `runId`. Other fields are type-specific.

The append must be atomic — partial writes on crash are not permitted. Implement by writing each event line followed by `fsync` (or Bun equivalent).

### 6. `state/current.json` (derived state — `code-oz` extension)
...
The M3 `src/state/gates.ts` module enforces:

1. Every gate file is valid JSON with `version: 1`.
...
5. `GATE_<PHASE>_PASSED.json` references an `artifact` that exists relative to the run root; if `artifactSha256` is set, the file's actual sha256 must match.
6. The orchestrator never advances a phase by reading agent output text — only by reading and validating the gate file.

Any validation failure is reported as a typed `GateLoadError` with `{ file, code, rule, detail? }` — the same shape as `AgentLoadError` from M2.
...
- Using `unlink` on gate files to "retry" — gate files are append-only artifacts of decisions. To redo a phase, increment the `runId`.
