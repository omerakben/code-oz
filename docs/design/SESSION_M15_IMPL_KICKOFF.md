...
Cost accepted: cross-family discipline is now baked into the bundled reviewer's permission shape. The reviewer's `tool_use.debate.opposingProviders` list must be populated such that every entry passes M11 eligibility AND differs in family from the run's BUILD provider AND from the reviewer's own provider family. Concrete scope (commit 8): `opposingProviders: [claude, gemini]` (the reviewer ships `provider: codex`; the M11-eligible cross-family pair is the canonical default-pool minus codex). `maxRounds: 1`, `timeoutMsPerRound: 120000`, `maxConcurrent: 1`, `network: provider-only`, `maxFiles: 16`, `maxTokens: 32000`. The exact YAML is locked in §11.8.
