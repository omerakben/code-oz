...
- **Post-M10 productization (locked sequence, one authority boundary per milestone, per CLAUDE.md rule 20). Sequence revised 2026-05-01 after the xAI expansion debate (`docs/design/SESSION_XAI_EXPANSION_KICKOFF.md`, thread `019de497`) — PE-1 inserted between M12 and M13:**
  ...
  - **M14 — Reviewer panel v1.** Authority boundary: panel quorum + cross-family enforcement + synthesis. First simultaneous-provider surface. Same-family panelists are advisory only; cross-family quorum is required for `verdict: ready`. REVIEW.md schema extension may be needed; subject to its own pre-implementation Codex debate. **Closed 2026-05-03 (v0.15.0-alpha.0 target, 2400 tests).** Trail: `docs/research/CODEX_BRIEFING_M14.md`, `docs/research/CODEX_RESPONSE_M14.md` (verdict accept-with-modifications, thread `019deb75`, 4 substantive pushbacks accepted), `docs/design/SESSION_M14_KICKOFF.md`, `docs/design/RALPH_M14_PROMPT.md`. **Rule-21 ship gate satisfied** — `bun run dev doctor --panel-baseline tests/fixtures/review-panel-baseline/baseline.json` emits `review_panel_baseline_completed` with `panelOnlyActionableFindingCount=1`, `sameFamilyVoteRejectionCount=1`, `manifestEqualityHeld=true`, `disagreementCount=1`; all four thresholds PASS. Built across 10 single-axis commits ending `569afb4`. Five-layer defense-in-depth in place against authority laundering: config-load + agent loader + artifact-parse + quorum-time + event-validator. Same-family advisory carries NO gate authority (positive or negative); ratification only via cross-family voter fingerprint match.
