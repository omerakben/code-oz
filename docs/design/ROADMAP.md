...
- **Post-M10 productization (locked sequence, one authority boundary per milestone, per CLAUDE.md rule 20). Sequence revised 2026-05-01 after the xAI expansion debate (`docs/design/SESSION_XAI_EXPANSION_KICKOFF.md`, thread `019de497`) — PE-1 inserted between M12 and M13:**
  ...
  - **M12 — Company roster for shipped roles only.** Authority boundary: role-to-provider routing. `docs/contracts/COMPANY.md` + `.code-oz/config.yaml` `company:` block mapping the six bundled personas (BA + Lead + Builder + Verifier + Reviewer + Scientist) to provider + model overrides. Locked six-name `M12_COMPANY_ROLES` constant — project-local personas with names outside this list are NOT routable as company roles in v0.1 (custom role routing is M16+). No per-role budgets (M13), no per-role permissions (deferred indefinitely; permissions stay persona-shaped), no Debate-opponent row (M15 governs the single-opponent `requestDebate()` runtime via per-persona `tool_use.debate.opposingProviders`), no Orchestrator row (orchestrator is the runtime, not a persona). Landed as `docs/contracts/COMPANY.md` + `M12_COMPANY_ROLES` + `CompanyConfig` in `src/config/schema.ts` + `mergeCompany` in `src/config/load.ts` (fail-closed on unsupported row keys + `loader_company_role_unknown` against the locked roster, per Codex Decision G) + `applyCompanyOverrides` in `src/agents/loader.ts` between bundled-vs-override merge and the resolved-provider checks + post-override debate-family check (Codex Risk #4 close) + bootstrap-before-loadConfig wiring fix in `src/cli/bootstrap.ts` and `src/commands/run.ts` at both call sites (Codex Risk #2 close) + `req.model ?? req.agent.model` defaulting in `src/providers/manifest.ts` + `agent_invoked.model` event field in `src/state/schemas.ts` for durable provenance (Codex Risk #3 close, also surfaces the latent frontmatter-model-drop that pre-dated M12). **Ships before PE-1 because Company roster is the product-thesis differentiator (the "AI software company" metaphor lands here); shipping M12 first means PE-1's xAI provider drops directly into a `company:` block on day one.** **Closed 2026-05-01 (v0.12.0-alpha.0, 1917 tests).**
  - **PE-1 — xAI direct HTTP adapter (demand-driven insertion).** Authority boundary: outbound HTTP from `code-oz` itself + API-key trust-boundary expansion. New provider/family/authSource value (`xai-api-key`); no `transport` field on `ProviderCapability` (M11 strict-minimal preserved); no gateway abstraction; xAI built-in server-side tools (web search, code execution) explicitly disabled by default until a future `tool_use` permission scope authorizes them. See `docs/design/SESSION_XAI_EXPANSION_KICKOFF.md` for the full pre-PE-1 contract additions and locks.
