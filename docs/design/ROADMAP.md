...
### Now (v0.21.0-alpha.0, branch `feat/m17-audit-runtime`, pending tag)

M17: AUDIT runtime for brownfield repositories. Implementation complete; pending R1 + R2 implementation review rounds before tag.
...
- 3742 offline tests, 0 fail, 2 pre-existing live-gated skips

### Next (post-M17 tag)

- M17 R1 + R2 implementation review; tag `v0.21.0-alpha.0`; push to origin + GitHub release
- v0.20.1 first-run polish release (was previously "Now"): README hero rewrite, provider claims corrected, trust docs, `.github/` templates, failure-mode demo, comparison page, Agent Gate Bench protocol, three `src/` polish fixes — see `v0_20_1_shipped_2026-05-14.md` in memory
...
- **Post-M10 productization (locked sequence, one authority boundary per milestone, per CLAUDE.md rule 20). Sequence revised 2026-05-01 after the xAI expansion debate (`docs/design/SESSION_XAI_EXPANSION_KICKOFF.md`, thread `019de497`) — PE-1 inserted between M12 and M13:**
  ...
  - **M17 — AUDIT runtime.** Authority boundary (rule 20): AUDIT runtime + dispatch + auditor persona + minimum PLAN consumption slice. The brownfield analog of DEFINE + SPEC + Lead-SPEC-read. New surfaces per commit: two-RED-check brownfield CLI e2e (C1); `dispatchAudit` in fresh-run + active-run paths (C2); `src/phases/audit.ts` skeleton + event sequence (C3); `operator problemStatement` gate on `run_started` + happy-path routing fix (C4-prep); `composeAuditPrompt` + composition-time universal-rules test + rule-16 CI grep guard + invocation wiring (C4-machinery, with fix-first tightening); `docs/contracts/AUDIT.md` artifact contract (C5a); `src/artifacts/audit-schema.ts` + `audit-parser.ts` + Scientist sidecar reuse + tightened validation (C5b, with fix-first hardening); `audit_completed` sha event + `preApproveAuditHook` (C6); AUDIT-to-PLAN handoff slice — `RunPlanOptions.profile` event-derived + `runPlan` brownfield fork + Lead persona inline brownfield section + `SC-AUDIT` grammar + `## Audit sources` heading (C7); hand-authored `auditor.md` + `audit-system.md` + `lead.md` brownfield section, authored by Ozzy + Claude line-by-line per rule 16 (C4-prose); full brownfield full-cycle e2e + greenfield regression (C8, with C8-fix closing 2 e2e-caught bugs). Greenfield flow is completely unchanged. **Two bugs surfaced only by the C8 full-cycle e2e** — per-commit fixture tests and all 10 Codex per-commit/seam review rounds passed, yet coupling bugs only appeared in the end-to-end run: (A) the `AUDIT ready-signal` was not stripped from the auditor prompt rendering (audit_completed fired but the signal leaked into the artifact text), and (B) the Scientist phase-tail was never wired into `runAudit` (HYPOTHESES.md + OPEN_QUESTIONS.md were never written, so `validateScientistSidecars` in `preApproveAuditHook` would have blocked every approve). Empirical evidence for rule 19 (milestone-level e2e is non-negotiable for state-machine work) and rule 20 (one authority per milestone). **10 Codex review rounds total:** anti-stub strictness (C1), seam reviews (C2, C6, C7, C4-prep, C8-fix), rule-16 leak guard (C4-machinery), implementation reviews (C5b, C3) — several fix-first verdicts, all findings closed before continuation; seam/implementation rounds converged push. Trail: `docs/design/SESSION_M17_KICKOFF.md`, `docs/research/CODEX_BRIEFING_M17.md` (R0 design), `docs/research/CODEX_RESPONSE_M17.md` (R0 verdict: `revise`), `docs/research/CODEX_BRIEFING_M17_R1.md` (R1 design), `docs/research/CODEX_RESPONSE_M17_R1.md` (R1 verdict: `accept-with-modifications`). **Closed locally 2026-05-21 (16 commits on `feat/m17-audit-runtime`, 3742 tests, 0 fail; tag pending R1/R2 impl review + push approval).**
  - **M18 — Reviewer Memory v1.** (Previously labelled M17.) Kickoff source for the memory-hygiene rubric: `docs/contracts/REVIEWER_MEMORY.md`.
  ...
  - **Template-comparison-derived deferred milestones (slots reserved 2026-05-10 gptme + 2026-05-12 opencode):**
    ...
    - **Candidate slot — Deny-dominant wildcard permission expressions (post-M16, demand-gated, not pre-locked; opencode B2).** Authority boundary: pattern-language extension to permission enforcement. Intersection guarantee preserved (request roots ⊆ agent.roots ⊆ permissions.read); inside the intersection, allow patterns expand the *expression* of allowed paths; any matching deny pattern overrides allow regardless of declaration order; `**` is forbidden outside the leaf segment; new event `permission_pattern_evaluated` records matched rule + final action. Rule-20 justification: a wildcard matcher that changes allow/deny semantics counts as a new authority surface even though the intersection invariant is preserved. Originated from the opencode comparison Codex fix-first (`docs/comparison/11-opencode/SYNTHESIS.md` §1, `docs/comparison/11-opencode/CODEX_RESPONSE.md` Q2). Codex-validated rationale: opencode's `findLast` last-match-wins is safe only under its `ask`-default world; in code-oz's `deny`-default world (rule 9), last-match-wins lets a permissive late rule silently override an earlier denial, which is exactly the bug class the intersection model is designed to prevent. Implementation surfaces (when prioritized): `src/agents/schema.ts` (new pattern fields under `permissions.read.patterns` and `permissions.tool_use.execute.patterns`; also the current home of execute permission shape itself — execute does not yet have a dedicated `src/tools/execute/permissions.ts`); `src/tools/repo-context/permissions.ts` (deny-dominant evaluator for repo-context); a future-extracted evaluator alongside the existing test-runner at `src/tools/test-runner.ts` if execute scope adopts patterns; `src/state/events.ts` (new event variant `permission_pattern_evaluated`). Slot is one of multiple post-M16 candidates surfaced by template comparisons; sequencing is the next planning round's job, not pre-locked here.
    - **Candidate slot — Cancellation, timeout, and debate-recursion guard (post-M16, demand-gated, not pre-locked; opencode M-CANCEL).** Authority boundary: structured cancellation throughout the run + panel-voter timeout + nested-`requestDebate` rejection. Three sub-surfaces under one umbrella because each is small but each is a new runtime authority and rule 20 prefers them split: (a) `runReview`, `runReviewPanel`, and `requestDebate` accept a top-level `AbortSignal` whose abort path guarantees `withLock`-based locks (`.review.lock`, `.build.lock`, `.verify.lock`, `.worktree.lock`) are released via try/finally even on user `^C`, with a new `run_aborted` event for observability; (b) `runReviewPanel` accepts a per-voter `timeoutMs`, fires `panel_voter_timeout` events, and refuses to write a partial canonical `REVIEW.md` when quorum cannot be formed before the deadline; (c) `requestDebate` adds a per-phase invocation-depth counter and rejects nested calls with a new typed `ProviderError('debate_recursion_blocked')` plus `NEEDS_INTERVENTION.json` per rule 11 (the existing topic-collision and concurrent-limit checks at `src/tools/debate-request.ts:276-283,406-414` are orthogonal — recursion fires before lock acquisition to avoid a lock-wait-within-lock deadlock). Originated from the opencode comparison Codex fix-first (`docs/comparison/11-opencode/SYNTHESIS.md` §3, `docs/comparison/11-opencode/CODEX_RESPONSE.md` Q5) where Codex agreed the N1 Effect rejection but flagged that the rejection lacks empirical backing without these three test classes. Test fixtures (`SlowProvider`, `HangProvider` implementing `IAgentProvider`) ship inside this slot, not before — shipping them without consumers would leave dead test infrastructure. Rule-20 sequencing: this slot ships a, b, and c as separate sub-milestones in three commits with per-commit Codex review, not as one bundle. Sequence: (a) AbortSignal first because it underpins the other two — without it, panel timeout is bookkeeping while provider work continues and may append late events; (b) panel timeout second, layered on top of AbortSignal as a deadline policy; (c) recursion guard last because its surface is the smallest but its placement (before lock acquisition at `src/tools/debate-request.ts:256`) is the easiest to get wrong.
  - Discipline: many agents may reason in parallel; only isolated builders mutate worktrees; only the orchestrator writes canonical artifacts and gates. CLAUDE.md rule 21 (Agentless-promoted-to-rule): no parallel-provider surface lands without measurable risk reduction in `events.jsonl` against the simpler baseline. The canonical measurement methodology lives in [`docs/contracts/RULE21_BENCHMARK.md`](../contracts/RULE21_BENCHMARK.md) (generalized from `DEBATE_POLICY.md`'s M15 application); any future M-series milestone proposing a new parallel-provider surface (M16+ deferred items above, PE-N parallel candidates, panel v2) must satisfy the gating metrics defined there before its ship gate fires.
