...
- **Idempotence + co-existence.** Inject an idempotent marker (`<!-- code-oz-router v1 -->`); the marker is a model-facing idempotence hint, and duplicate injection is NOT suppressed by the (stateless) hook. When superpowers (or any other bootstrap) is installed, code-oz's card defers to user instructions and does not contest superpowers' skill-routing; it adds only the engine-routing pointer. No coercive "1%/no choice" language.
