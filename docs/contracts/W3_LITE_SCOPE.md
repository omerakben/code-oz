...
3. **Handoff bundle.** `dist/handoff/` directory contains `darwin-arm64/code-oz`, `darwin-x64/code-oz`, `install.sh`, `manifest.json`, and `README.md` with one-page operator-friendly instructions. The locked layout is `dist/handoff/{darwin-arm64,darwin-x64}/code-oz`, not `dist/handoff/bin/<arch>/code-oz`. The tarball at `dist/code-oz-v0.14.0-alpha.0-darwin.tar.gz` contains a root directory `code-oz-v0.14.0-alpha.0-darwin/` that mirrors the handoff layout. Friends receive this directory as a tarball.
...
- **Permissions Codex MAY use:** read repo, write files matching `scripts/**`, `dist/**`, `tests/**`, and the W3-lite docs paths; run `bun test`, `bun run typecheck`, `bun run build:binaries`, `bun build --compile`, `git status`, `git diff`. Codex MAY NOT push, tag, merge, fetch, pull, or run `gh` commands.
- **Git commit ownership:** the codex-native MCP `workspace-write` sandbox blocks `.git/index.lock` writes, so Codex sub-agents cannot run `git add` or `git commit` in practice. The Opus orchestrator stages and commits each iteration's work after Codex's verify checks pass. Each commit message names the Codex thread id for audit-trail purposes.
- **Per-iteration deliverable:** the Opus orchestrator appends a one-section summary to `.code-oz/state/ralph-state.md` (gitignored, session-local) after each iteration. `.code-oz/state/ralph-summary.md` is the durable halt artifact written when the loop converges.
