...
| Reviewer panel | M14 in flight (v0.15.0-alpha.0 target; R1 fix-first) | Panel quorum + cross-family enforcement; sidecar surface complete; production REVIEW lifecycle integration pending Codex R1 block-push closure |
...
| M14 (in flight) | Reviewer panel v1 — first simultaneous-provider surface. Panel quorum + cross-family enforcement (same-family panelists are advisory only, with NO gate authority — Codex R0 pushback Q7) + orchestrator-owned synthesis. Sidecar surface (pure verdict helper, schema, events, baseline command) delivered by Ralph overnight; production REVIEW lifecycle integration (runReview dispatch, approve.ts mode-aware parser, registry-owned family resolution) is pending Codex R1 block-push closure. |
